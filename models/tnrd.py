...
class TNRDlayer(nn.Module):
    ...
    def forward(self,input):
        ...
        for it in range(1):
            ...
            def omega_alpha(l, Alpha):
                return l*(1-Alpha) - (l-1)*(1-Alpha)

            for k in range(2, max_iter+1):
                u_new = np.zeros_like(u)  # Placeholder for the updated image

                summation = 0
                for l in range(1, k):
                    summation += (omega_alpha(l, Alpha) - omega_alpha(l+1, Alpha)) * u


            u_new = summation - omega_alpha(k, Alpha) * noisy + output.mul(beta).sum(1,keepdim=True)-self.alpha.exp()*(u-f)
